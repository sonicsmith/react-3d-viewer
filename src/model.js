...
import {
  DirectionLight as directionligth,
...
var GlEment = {
  directionligth: {
    create({ color, position, castShadow } = {}) {
      var direction_light = new THREE.DirectionalLight(
...
class Model extends React.Component {
  ...
  get need_children_lights() {
    if (
      this.array_children.filter(o => {
        return /directionligth|ambientlight|spotlight/.test(
          o.props.__constructor
        )
      }).length
    ) {
      return true
  ...
  initControl() {
    ...
    this.orbit_controls = new THREE.OrbitControls(
      this.camera,
